...
final class Server[F[_]: Concurrent: Logger] private[kitteh] (
  ...
  def eval(
    ...
    val body = cmd match {
      ...
      case Get(key) =>
        Stream.eval(world.get) flatMap { case World(data, _) =>
          ...
          if (!filtered.isEmpty) {
            ...
            val eff = makeTopics *> makeSignals flatMap { signals =>
              val update = state update { st =>
                signals.foldLeft(st) { case (st, (ch, d)) =>
                ...
                val subscriptions = Stream evals {
                  ...
                  state.get.map(_.subscriptions.size) map { num =>
                    signals map { case (ch, d) =>
                      ...
                      Stream.emit(Right(confirmation)) ++ topic
                        .subscribe(MaxOutstanding)
                        .interruptWhen(
                          d
                        ) // wire up the kill switch to ensure Unsubscribe works
      ...
      case Unsubscribe(channels) =>
        ...
        val toRemove = if (channels.isEmpty) {
          ...
          state modify { st =>
            val (subset, subs2) =
              channels.foldLeft((Map[String, F[Unit]](), st.subscriptions)) {
                case ((acc, subs), ch) =>
                  subs.get(ch) match {
        ...
        val back = Stream eval {
          state.get.map(_.subscriptions.size) map { before =>
            unsubbed.zipWithIndex map { case (ch, i) =>
              Right(
                RESP.Array.Full(
                  List(
...
object Server {
  ...
  final case class World[F[_], A, B](
      data: Map[A, B],
      pubsub: Map[A, Topic[F, B]]
  )
